...
import { effectiveMode, hugs, reservedRows, scrolls, sizeAll } from '$lib/layout/grid/sizing';
import { UNIT, GAP, unitsFor } from '$lib/layout/grid/units';
import type { Intent, Layout } from '$lib/layout/grid/types';
...
describe('unitsFor', () => {
	it('counts the pane inset in, so a card of N units of card is N units of grid', () => {
		expect(unitsFor(6 * UNIT - GAP)).toBe(6);
	});

	it('rounds a part-used row up, and never reports less than one', () => {
		expect(unitsFor(6 * UNIT - GAP + 1)).toBe(7);
...
describe('reservedRows', () => {
	it('a chart reserves the height it was given, whatever it measured', () => {
		expect(reservedRows(intent(), 'scale', 4000, false)).toBe(8);
	});

	it('a fixed list reserves its set height, even with two rows in it', () => {
		expect(reservedRows(intent({ mode: 'fixed' }), 'flow', 40, false)).toBe(8);
	});

	it('a fitted list reserves what it measured, without limit', () => {
		const tall = 30 * UNIT - GAP;
		expect(reservedRows(intent({ mode: 'fit' }), 'flow', tall, false)).toBe(30);
	});

	it('a capped list stops at its ceiling', () => {
		const tall = 30 * UNIT - GAP;
		expect(reservedRows(intent({ mode: 'cap', cap: 12 }), 'flow', tall, false)).toBe(12);
	});

	it('a capped list under its ceiling reserves only what it uses — in normal mode', () => {
		const short = 5 * UNIT - GAP;
		expect(reservedRows(intent({ mode: 'cap', cap: 12 }), 'flow', short, false)).toBe(5);
	});

	it('but reserves the WHOLE ceiling while arranging, so nothing is placed in its room to grow', () => {
		const short = 5 * UNIT - GAP;
		expect(reservedRows(intent({ mode: 'cap', cap: 12 }), 'flow', short, true)).toBe(12);
	});

	it('falls back to the authored height before the first measurement', () => {
		expect(reservedRows(intent({ mode: 'fit', h: 9 }), 'flow', undefined, false)).toBe(9);
	});
});
...
describe('sizeAll', () => {
	...
	it('resolves each pane by its own kind and keeps the priority order', () => {
		const sized = sizeAll(
			[intent({ id: 'list', mode: 'fit', h: 6 }), intent({ id: 'chart', h: 10 })],
			specs,
			{ list: 20 * UNIT - GAP },
			false
		);
